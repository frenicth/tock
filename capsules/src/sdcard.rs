...
impl<'a, A: hil::time::Alarm + 'a> SDCardClient for SDCardDriver<'a, A> {
    fn card_detection_changed(&self, installed: bool) {
        self.app_state
            .map(|app_state| {
                app_state.callback
                    .map(|mut cb| { cb.schedule(0, installed as usize, 0); });
            });
    }
    ...
    fn read_done(&self, data: &'static mut [u8], len: usize) {
        ...
        self.app_state
            .map(|app_state| {
                ...
                app_state.callback
                    .map(|mut cb| { cb.schedule(2, read_len, 0); });
            });
    }
    ...
    fn error(&self, error: u32) {
        self.app_state
            .map(|app_state| {
                app_state.callback
                    .map(|mut cb| { cb.schedule(4, error as usize, 0); });
            });
    }
}
...
impl<'a, A: hil::time::Alarm + 'a> Driver for SDCardDriver<'a, A> {
        ...
        match allow_num {
            ...
            0 => {
                if self.app_state.is_none() {
                    ...
                    self.app_state
                        .map(|appst| { appst.read_buffer = Some(slice); });
                }
                ReturnCode::SUCCESS
            }
    ...
    fn command(&self, command_num: usize, data: usize, _: AppId) -> ReturnCode {
        match command_num {
            ...
            4 => {
                self.app_state
                    .map_or(ReturnCode::ENOMEM, |app_state| {
                        app_state.write_buffer
                            .as_mut()
